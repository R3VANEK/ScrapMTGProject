...
create procedure insertData(
        ...
        IN card_name1 varchar(60), IN card_image1 varchar(150), IN mana_cost1 varchar(9),
        IN converted_mana_cost1 Integer, IN card_number1 Integer, IN card_type1 varchar(40),
        IN rarity1 varchar(11), IN power1 varchar(4), IN toughness1 varchar(4),
        IN artist_name varchar(255), IN price1 Decimal, IN id_expansion1 INTEGER           
    )
    begin
        start transaction;
        set transaction isolation level serializable;
            ...
            @card_id = select max(id_card) from cards;
            @ampLocation = Locate("&", artist_name);

            if @ampLocation <> 0 then

                @firstArtist = substring(artist_name ,1 ,@ampLocation-1);
                @secondArtist = substring(artist_name, @ampLocation+1, length(@trimmed));
                insert ignore into artists(name) values (@firstArtist), (@secondArtist);

                @firstArtistID = select max(id_artist)-1 from artists;
                @secondArtistID = select max(id_artist) from artists;
                ...
                @artist_id = select max(id_artist) from artists;
